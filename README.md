...
> **Custom hosted landing page:** set `TILLER_CUSTOM_SITE_ENABLED=true` and place your site at `./data/site/index.html` (with optional CSS, JavaScript, and media files alongside it). The custom site replaces only the hosted landing page and public assets; `/app`, authentication, API, health, and platform routes remain application-owned. When disabled or unset, the embedded landing page is used. Enabling it requires `data/site/index.html` to exist at startup.
